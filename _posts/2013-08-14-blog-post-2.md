...
To start, we store the vector \\( a_1 \\) temporarily. Afterwards, we define \\( q \\) as equal to \\( \left \lceil{a/b}\right \rceil \\) where \\( a,b \\) are the first value in the vectors \\( a_0 \\) and \\( a_1 \\) respectively.
